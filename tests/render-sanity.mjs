...
try {
  ...
  assert.doesNotMatch(html, /Vacas de oportunidade · receita adicional de A/);
  assert.doesNotMatch(html, /Incluir vacas de oportunidade|Janela das vacas validada|Compra da vaca magra/);
  assert.doesNotMatch(html.replace(/<!--.*?-->/g, ''), /\+\s*798 vacas/);
  assert.match(html, /não gera crédito adicional/);
  ...
  assert.match(html, /Com o mesmo gasto, qual deixa mais margem/);
  assert.match(html.replace(/<!--.*?-->/g, ''), /238,15 ha/);
  assert.match(html, /Reserva operacional estimada/);
  const { EqualCattleCostPanel } = await server.ssrLoadModule('../components/equal-cattle-cost.tsx');
  const { equalCattleCost } = await server.ssrLoadModule('../lib/equal-cattle-cost.ts');
  const { photoReferenceAssumptions } = await server.ssrLoadModule('../lib/photo-reference.ts');
  const saturatedHtml = renderToString(React.createElement(EqualCattleCostPanel, {study: equalCattleCost({...photoReferenceAssumptions(),totalArea:1050})}));
  assert.match(saturatedHtml,/cocho atual limita/);
  assert.match(saturatedHtml,/limite não utilizado/);
  assert.doesNotMatch(saturatedHtml,/deixa .* a mais por ano/,'não proclamar igualdade de custo quando o cocho limita');
